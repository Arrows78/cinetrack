...
pub async fn device_id(pool: &SqlitePool) -> Result<String, ApiError> {
    if let Some((value,)) = sqlx::query_as::<_, (String,)>(
...
pub async fn status(pool: &SqlitePool) -> Result<SyncStatus, ApiError> {
    let profile_id = current_profile_id(pool).await?;
    let pending_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM sync_outbox WHERE profile_id=?1")
        .bind(&profile_id).fetch_one(pool).await.map_err(ApiError::from)?;
...
pub async fn list_outbox(pool: &SqlitePool, limit: i64) -> Result<Vec<SyncOutboxMutation>, ApiError> {
    let profile_id = current_profile_id(pool).await?;
    let limit = limit.clamp(1, 200);
    let rows: Vec<(String,String,String,String,Option<String>,i64,String,i64)> = sqlx::query_as(
    ...
    rows.into_iter().map(|row| {
        let payload = row.4.map(|raw| serde_json::from_str(&raw))
            .transpose().map_err(|error| ApiError::internal(format!("Invalid sync payload: {error}")))?;
        Ok(SyncOutboxMutation {
            mutation_id: row.0, entity_type: row.1, entity_id: row.2,
            operation: row.3, payload, base_version: row.5,
            created_at: row.6, attempt_count: row.7,
        })
    }).collect()
}
...
pub async fn ack_mutations(pool: &SqlitePool, acks: &[SyncMutationAck]) -> Result<(), ApiError> {
    ...
    let mut tx = pool.begin().await.map_err(ApiError::from)?;
    for ack in acks {
        if !validate_entity_type(&ack.entity_type) || ack.version <= 0 { continue; }
        let now = now_iso(&mut *tx).await?;
        sqlx::query(
        ...
        sqlx::query("DELETE FROM sync_outbox WHERE profile_id=?1 AND mutation_id=?2")
            .bind(&profile_id).bind(&ack.mutation_id).execute(&mut *tx).await.map_err(ApiError::from)?;
...
pub async fn rebase_conflicts(pool: &SqlitePool, conflicts: &[SyncConflict]) -> Result<(), ApiError> {
    let profile_id = current_profile_id(pool).await?;
    let mut tx = pool.begin().await.map_err(ApiError::from)?;
    for conflict in conflicts {
        if !validate_entity_type(&conflict.entity_type) || conflict.server_version < 0 { continue; }
        sqlx::query(
...
async fn delete_entity(
    tx: &mut Transaction<'_, Sqlite>, profile_id: &str, entity_type: &str, entity_id: &str,
) -> Result<(), ApiError> {
    let sql = match entity_type {
        ...
        "custom_list_item" => "DELETE FROM custom_list_items WHERE uuid=?2 AND list_id IN (SELECT uuid FROM custom_lists WHERE profile_id=?1)",
        _ => return Err(ApiError::bad_request("Unsupported sync entity type")),
    };
    sqlx::query(sql).bind(profile_id).bind(entity_id).execute(&mut **tx).await.map_err(ApiError::from)?;
    Ok(())
}

async fn upsert_entity(
    tx: &mut Transaction<'_, Sqlite>, profile_id: &str, entity_type: &str, data: &serde_json::Value,
) -> Result<(), ApiError> {
    let payload = serde_json::to_string(data)
        .map_err(|error| ApiError::bad_request(format!("Invalid remote payload: {error}")))?;
    let sql = match entity_type {
        "library_item" => r#"INSERT INTO library_items(uuid,profile_id,media_id,media_type,title,poster_path,backdrop_path,year,rating,genres,status,favourite,user_rating,notes,tags,started_at,completed_at,rewatch_count,created_at,updated_at)
        ...
        "seen_movie" => r#"INSERT INTO seen_movies(uuid,profile_id,movie_id,title,poster_path,backdrop_path,watched_at,created_at,updated_at)
        ...
        "episode_progress" => r#"INSERT INTO episode_progress(uuid,profile_id,series_id,episode_id,season_number,episode_number,watched,watched_at,created_at,updated_at)
        ...
        "tracked_series" => r#"INSERT INTO tracked_series(uuid,profile_id,series_id,title,poster_path,backdrop_path,total_episodes,created_at,updated_at,status)
        ...
        "viewing_event" => r#"INSERT INTO viewing_events(uuid,profile_id,media_id,media_type,title,event_type,watched_at,duration_minutes,episode_id,season_number,episode_number,created_at,note)
        ...
        "custom_list" => r#"INSERT INTO custom_lists(uuid,profile_id,name,description,created_at,updated_at)
        ...
        "custom_list_item" => r#"INSERT INTO custom_list_items(uuid,list_id,media_id,media_type,title,poster_path,position,added_at,updated_at)
        ...
        "smart_list" => r#"INSERT INTO smart_lists(uuid,profile_id,name,rules,created_at,updated_at)
        ...
        "saved_filter" => r#"INSERT INTO saved_filters(uuid,profile_id,page,name,filters,created_at,updated_at)
        ...
        "availability_alert" => r#"INSERT INTO availability_alerts(uuid,profile_id,media_id,media_type,title,region,provider_ids,enabled,created_at,updated_at)
    ...
    sqlx::query(sql).bind(payload).bind(profile_id).execute(&mut **tx).await.map_err(ApiError::from)?;
    Ok(())
}

pub async fn apply_remote_changes(pool: &SqlitePool, changes: &[RemoteSyncChange]) -> Result<(), ApiError> {
    ...
    for change in &ordered {
        if change.sequence <= max_sequence || !validate_entity_type(&change.entity_type) { continue; }
        ...
        if pending.is_none() {
            match change.operation.as_str() {
                "delete" => delete_entity(&mut tx, &profile_id, &change.entity_type, &change.entity_id).await?,
                "upsert" => {
                    let data = change.data.as_ref().ok_or_else(|| ApiError::bad_request("Remote upsert has no payload"))?;
    ...
    ).bind(cursor_key(&profile_id)).bind(max_sequence.to_string()).bind(now)
     .execute(&mut *tx).await.map_err(ApiError::from)?;
