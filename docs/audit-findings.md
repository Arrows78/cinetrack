...
- [x] `list_library`/`list_watchlist` unbounded query — both now bound by a `LIST_SAFETY_LIMIT = 5000` in `library.rs`/`watchlist.rs`. This is a safety cap, not true server-side pagination: the frontend still loads and filters the whole bounded set client-side (the grid itself is virtualized via `react-virtuoso`). True cursor pagination would require duplicating all client-side filter/sort logic into Rust — judged excessive complexity for a single-user local app at this scale.
