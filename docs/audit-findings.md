...
- **Counter-example found:** `docs/database-schema.md` documents that accessing a profile "requires being signed in with that specific account," but `update_preference` (`src-tauri/src/commands/preferences.rs`) accepts any key/value pair — `active_profile_id` is never validated against the authenticated Supabase session — and `current_profile_id()` (`src-tauri/src/database/mod.rs`), used by every profile-scoped command, just re-reads that preference with no authorization check. A direct `invoke("update_preference", { key: "activeProfileId", value: "default" })` call bypasses the documented guarantee.
- **Rule:** a command that reads or writes profile-scoped (or otherwise access-controlled) data must itself verify the caller may act on that resource. Follow the pattern already used correctly for custom lists and availability alerts (`assert_owns_list` in `src-tauri/src/commands/custom_lists.rs`, the ownership check in `availability.rs`) — extend the same idea to profile switching itself when auth is required.
...
- [ ] `calendarService.build()` (`src/features/calendar/calendar-service.ts`) only checks the first page of upcoming movies, the first 20 tracked series, and each series' last 2 seasons, with no on-screen indicator that the calendar is truncated.
- [ ] `pickBestMatch` in the TV Time importer (`src/features/tvtime/tvtime-import-service.ts`) falls back to the first TMDB search result with no confidence threshold when no exact title+year match exists — can silently attach the wrong title.
