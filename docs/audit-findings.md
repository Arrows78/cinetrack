...
- [x] `list_library` unbounded query — bound by a `LIST_SAFETY_LIMIT = 5000` in `library.rs`. This is a safety cap, not true server-side pagination: the frontend still loads and filters the whole bounded set client-side (the grid itself is virtualized via `react-virtuoso`). True cursor pagination would require duplicating all client-side filter/sort logic into Rust — judged excessive complexity for a single-user local app at this scale. (`watchlist_items`/`list_watchlist` no longer exist — migration 10 folded the watchlist feature into `library_items`, see the "Merged the watchlist feature into the library" entry below.)
