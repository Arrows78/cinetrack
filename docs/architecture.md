...
**What's generated vs. hand-written across the IPC boundary.** `pnpm contract:check` (part of `pnpm validate:frontend`) runs two checks: `scripts/generate-tauri-command-names.mjs --check` diffs `src/generated/tauri-command-names.ts` against `tauri::generate_handler![...]` in `src-tauri/src/lib.rs`, so a command typo or a command registered in Rust but never wired into a TS `defineCommand(...)` call fails the build. `scripts/check-contract-drift.mjs` covers a different, narrower gap: for an explicit list of Rust `pub struct` DTOs, it compares the field-_name_ set (after `snake_case` → `camelCase`) against the hand-written TS `interface` mirroring it in `src/types/media.ts`, so a field renamed/added/removed on one side and not the other fails the build too. Neither script generates TS _types_ from Rust — `Args`/`Result` shapes (e.g. `SaveLibraryItemArgs` in `library-commands.ts` vs. Rust's `MediaSummaryInput`/`LibraryPatch` parameters) are still written by hand on both sides, with only the field-name check above catching drift in the ones listed in `check-contract-drift.mjs`'s `PAIRS`. A full Rust→TS type generator (`tauri-specta` is the obvious candidate) was evaluated and deliberately not adopted: as of this writing it's still `2.0.0-rc.25`, unreleased as stable for over a year, with a documented history of breaking against recent Tauri versions — revisit once it stabilizes. Add a new DTO to `PAIRS` whenever it's non-trivial enough that a silent field drift would be easy to miss in review.
