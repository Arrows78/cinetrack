...
- **`src-tauri/src/sync/`** — the local half. Migration `018-add-sync-outbox.sql` adds `sync_outbox`/`sync_metadata`/`sync_entity_state`/`sync_control` plus an `AFTER INSERT/UPDATE/DELETE` trigger on each of 11 business tables (`library_items`, `seen_movies`, `episode_progress`, `tracked_series`, `viewing_events`, `custom_lists`, `custom_list_items`, `smart_lists`, `saved_filters`, `availability_alerts`, `dismissed_recommendations` — the last added by migration `020-add-dismissed-recommendations.sql`) that captures every mutation into `sync_outbox` **inside the same transaction** as the mutation itself — a crash between the two can't happen. `episode_progress`'s own `rating` column is the one deliberate exception: real, but excluded from that table's sync payload (see `docs/database-schema.md`). `sync_control.suppress_outbox` (flipped for the duration of `apply_remote_changes`, and around a profile deletion cascade) stops a trusted remote write from re-queuing itself as if it were a new local change.
