...
This document is the detailed, code-grounded companion to the architecture summary in `CLAUDE.md`. Read `docs/design-system.md` for UI/token rules and `docs/auth.md` for Clerk identity + Third-Party Auth — this document covers how data flows through the app and how a domain is structured end to end.
...
CineTrack is local-first: SQLite stays the source of truth for the device it's on, and cloud sync is a convergence layer on top, not a replacement for it. The engine spans three places:

- **`src-tauri/src/sync/`** — the local half. Migration `018-add-sync-outbox.sql` adds `sync_outbox`/`sync_metadata`/`sync_entity_state`/`sync_control` plus an `AFTER INSERT/UPDATE/DELETE` trigger on each business table (`library_items`, `seen_movies`, `episode_progress`, `tracked_series`, `viewing_events`, `custom_lists`, `custom_list_items`, `smart_lists`, `saved_filters`, `availability_alerts`, plus `dismissed_recommendations` in `020-add-dismissed-recommendations.sql` and `activity_log` in `021-sync-activity-log-and-episode-rating.sql`) that captures every mutation into `sync_outbox` **inside the same transaction** as the mutation itself — a crash between the two can't happen. `episode_progress.rating` is part of that payload from migration 21. `sync_control.suppress_outbox` (flipped for the duration of `apply_remote_changes`, and around a profile deletion cascade) stops a trusted remote write from re-queuing itself as if it were a new local change.
- **Supabase (`supabase/migrations/`)** — not part of the Cargo/pnpm build; applied with `supabase db push`. Postgres here is only the sync/community backend: Clerk issues identity (`docs/auth.md`). `sync_documents` holds one authoritative row per `(user_id, entity_type, entity_id)` with a monotonic `version`; `sync_changes` is the append-only, sequence-numbered change feed every device pulls from; `sync_mutations` deduplicates a retried push by `mutation_id`. `apply_sync_batch(device_id, mutations)` is the one RPC every push goes through — it checks each mutation's `baseVersion` against the current server version and returns either an ack (with the new version + sequence) or a conflict (the server's current version/data) per mutation, never a bare success/failure for the whole batch. `pull_sync_changes(after, limit)` is the cursor-based read side. Every table is RLS-scoped to `(select requesting_user_id())` (JWT `sub`, a Clerk id like `user_xxx`) — never a `user_id` read off the request body, and never `auth.uid()` (Clerk ids are not UUIDs). That matches the same "don't trust a client-supplied identity string" rule `preferences::service::set_active_profile` already applies to `supabase_user_id` locally (column name kept; the value is the Clerk `sub`).
- **`src/features/sync/`** — `sync-service.ts`'s `execute()` (prepare → push → pull → re-push, since a pull can rebase a pending local edit onto a newer version) is what `initialize()` runs on a timer, on `online`, and on a Supabase Realtime `postgres_changes` wakeup — Realtime is deliberately just a "something changed, go sync" nudge, never the transport itself, so a device that's asleep or offline when the event fires still converges correctly on its next cursor-based pull. The Realtime filter is `user_id=eq.${clerkUserId}` using the same `sub` as `requesting_user_id()`. `App.tsx` calls `syncService.initialize(queryClient)` once, gated on an active Clerk session. The data client is `src/shared/lib/supabase-data-client.ts` (`createClient` + `accessToken` from `session.getToken()`, no JWT template); `supabase.auth.getUser()` is disabled in that mode.

**Entity identity, and its one real edge case.** Most synced entities use their local `uuid` as the identity `apply_remote_changes` upserts by. Four of them — `library_item`, `seen_movie`, `episode_progress`, `tracked_series` — also carry a `UNIQUE(profile_id, ...)` business-key constraint (see `docs/database-schema.md`), so `upsert_entity` (`sync/service.rs`) resolves those specifically by that business key instead: two devices that each added the same title before ever syncing generate two different uuids for it, and resolving by uuid would hit that UNIQUE constraint and abort the whole batch. The trade-off is documented in `upsert_entity`'s own comment: the two devices converge on field values through whichever row already existed locally, without the two ever agreeing on one canonical uuid server-side. `account_preferences` is a fifth special case: `preferences` has no `uuid`/`profile_id` column at all (it's a single global key/value table), so it's captured directly in `write_preference` rather than via a SQLite trigger, and only for the keys in `ACCOUNT_SCOPE_PREFERENCE_KEYS` (`preferences/models.rs`) — the rest (`theme`, `compactMode`, `backupDirectory`, `activeProfileId`, ...) describe this installation, not the person, and must never leave it.
...
- `vitest.config.ts` pins coverage thresholds **per file**, not globally. The current suite covers the application broadly; keep thresholds aligned with the actual coverage of each touched file and investigate regressions rather than weakening a threshold silently.
