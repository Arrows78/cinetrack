...
- **`src-tauri/src/sync/`** — the local half. Migration `018-add-sync-outbox.sql` adds `sync_outbox`/`sync_metadata`/`sync_entity_state`/`sync_control` plus an `AFTER INSERT/UPDATE/DELETE` trigger on each of 9 business tables (`library_items`, `seen_movies`, `episode_progress`, `tracked_series`, `viewing_events`, `custom_lists`, `custom_list_items`, `smart_lists`, `saved_filters`, `availability_alerts`) that captures every mutation into `sync_outbox` **inside the same transaction** as the mutation itself — a crash between the two can't happen. `sync_control.suppress_outbox` (flipped for the duration of `apply_remote_changes`, and around a profile deletion cascade) stops a trusted remote write from re-queuing itself as if it were a new local change.
