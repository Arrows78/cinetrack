...
- **Supabase (`supabase/migrations/`)** — not part of the Cargo/pnpm build; applied with `supabase db push`. `sync_documents` holds one authoritative row per `(user_id, entity_type, entity_id)` with a monotonic `version`; `sync_changes` is the append-only, sequence-numbered change feed every device pulls from; `sync_mutations` deduplicates a retried push by `mutation_id`. `apply_sync_batch(device_id, mutations)` is the one RPC every push goes through — it checks each mutation's `baseVersion` against the current server version and returns either an ack (with the new version + sequence) or a conflict (the server's current version/data) per mutation, never a bare success/failure for the whole batch. `pull_sync_changes(after, limit)` is the cursor-based read side. Every table is RLS-scoped to `auth.uid()` — never a `user_id` read off the request body, matching the same "don't trust a client-supplied identity string" rule `preferences::service::set_active_profile` already applies to `supabase_user_id` locally.
- **`src/features/sync/`** — `sync-service.ts`'s `execute()` (prepare → push → pull → re-push, since a pull can rebase a pending local edit onto a newer version) is what `initialize()` runs on a timer, on `online`, and on a Supabase Realtime `postgres_changes` wakeup — Realtime is deliberately just a "something changed, go sync" nudge, never the transport itself, so a device that's asleep or offline when the event fires still converges correctly on its next cursor-based pull. `App.tsx` calls `syncService.initialize(queryClient)` once, gated on an active Supabase session.
