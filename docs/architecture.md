...
**Local timing, no remote telemetry.** Every command's body is wrapped in `src-tauri/src/diagnostics.rs`'s `timed("command_name", async { ... }).await`, which logs `command=<name> duration=<ms>ms` (or `slow_command=` at warn level past a threshold, `status=error` appended on failure) to the same `logs/cinetrack.log` file the frontend's own `invokeCommand()` writes to (see `src/shared/lib/invoke.ts`) — the exact same line shape on both sides, so the two interleave by timestamp when read together, and Settings' diagnostics view (raw tail + a structured `export_diagnostics_summary` aggregate) shows both without knowing which side produced which line. `timed` reads a process-wide `OnceLock<PathBuf>` set once from `.setup()` rather than taking an `AppHandle` parameter — every command-level test in this crate (`tauri::test::mock_app()`-based) calls its command directly by name, so threading a parameter through every signature would have meant updating all of them; the static stays unset under `cargo test`, making logging a silent no-op there instead. This is intentionally not a distributed trace ID: Tauri's IPC has no generic channel to thread one through every command without a much bigger change, so command name + roughly-simultaneous timestamps are the practical correlation between the frontend and backend's log lines.

**What's generated vs. hand-written across the IPC boundary.** `pnpm contract:check` (part of `pnpm validate:frontend`) runs three checks. `scripts/generate-tauri-command-names.mjs --check` diffs `src/generated/tauri-command-names.ts` against `tauri::generate_handler![...]` in `src-tauri/src/lib.rs`, so a command typo or a command registered in Rust but never wired into a TS `defineCommand(...)` call fails the build. `scripts/check-contract-drift.mjs` covers a different, narrower gap: for an explicit list of Rust `pub struct` DTOs, it compares the field-_name_ set (after `snake_case` → `camelCase`) against the hand-written TS `interface` mirroring it in `src/types/media.ts`, so a field renamed/added/removed on one side and not the other fails the build too. `scripts/check-tauri-command-signatures.mjs` checks a third gap: for every registered command it compares each Rust parameter (name, optionality, and a coarse shape category — string/number/boolean/void/array, with `named` as the catch-all for everything else, including every struct/enum/object — treating `State<...>`/`AppHandle`/`Window`/`WebviewWindow` as framework-injected rather than caller-supplied) against the matching TS `defineCommand(...)` `Args`/`Result` descriptor, so an added/removed/renamed parameter, or one that's optional on only one side, fails the build too. None of the three generates TS _types_ from Rust, and the signature checker's shape categories are coarse by design: a mismatch is only flagged when _neither_ side is `named`, so a `named` shape paired with any other category — two structurally unrelated Rust structs, a Rust enum against an unrelated TS union, an inner `Option<T>`/`Vec<SomeDto>` no longer matching across the boundary — still passes silently. A full Rust→TS type generator (`tauri-specta` is the obvious candidate) was evaluated and deliberately not adopted: as of this writing it's still `2.0.0-rc.25`, unreleased as stable for over a year, with a documented history of breaking against recent Tauri versions — revisit once it stabilizes. Until then, add a new DTO to `check-contract-drift.mjs`'s `PAIRS` whenever it's non-trivial enough that a silent field drift would be easy to miss in review.
...
- A feature may depend on another feature only through that feature's public surface: `*-repository.ts`, `use-*.ts`, or a curated `index.ts`. Command descriptors, services, API adapters, evaluators and other helpers stay private to their owning feature.
...
`eslint.config.js`'s `boundaries/dependencies` rule (via `eslint-plugin-boundaries`) enforces the `shared ↛ features/pages` and `components/ui ↛ features/pages` rules as build-breaking errors — `pnpm lint` is the check. Feature-to-feature isolation **is** enforced, separately: `pnpm architecture:check` (`scripts/check-feature-boundaries.mjs`, part of `pnpm validate:frontend`) walks the TS program and fails the build if one feature imports another feature's file other than its public surface (`*-repository.ts`, `use-*.ts`, or `index.ts`) — see rule 3 above. Every one of the 16 `src/features/*/` directories has its own `__tests__/` subdirectory; some (e.g. `media/`) additionally have their own internal subdirectories (`api/`), which this check treats as private.
