...
// Compact profile switcher for the persistent nav chrome (sidebar header,
// mobile header) so switching doesn't require a trip to Settings. The
// switching logic itself — including the security-critical branch below —
// is copied faithfully from ProfilesCard in settings-page.tsx; that
// component stays the canonical place to create/delete profiles, this one
// only switches between profiles that already exist.
export function ProfileSwitcher({ collapsed = false }: { collapsed?: boolean }) {
  ...
  // on the Settings page — this compact nav control just stays hidden
  // rather than showing broken/empty chrome on every screen, but the
  // failure itself still gets logged instead of disappearing silently.
  if (profiles.isError) {
