...
export function ProviderAvailability({ media }: { media: MediaSummary }) {
  ...
  return <section className="rounded-3xl border border-border bg-card/60 p-5"><h2 className="font-semibold">Disponible en streaming · {region}</h2><div className="mt-4 flex flex-wrap gap-3">{providers.map((provider) => <div key={provider.id} className="flex items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm">{provider.logoPath ? <img className="size-7 rounded-lg" src={buildTmdbImageUrl(provider.logoPath,"w92") ?? undefined} alt=""/> : null}<span>{provider.name}</span></div>)}</div></section>;
}
