...
export function StatsPage() {
  ...
  return (
    <div className="space-y-8">
      <header><h1 className="font-display text-3xl font-bold">{t("stats.title")}</h1><p className="mt-1 text-muted-foreground">{t("stats.description")}</p></header>
      ...
      <section className="rounded-3xl border border-border bg-card/60 p-5"><h2 className="font-semibold">{t("stats.activity12Months")}</h2><div className="mt-5 flex h-44 items-end gap-2">{stats.data.monthlyActivity.map((month) => <div key={month.month} className="flex min-w-0 flex-1 flex-col items-center gap-2"><div className="w-full rounded-t-lg bg-primary/80" style={{height:`${Math.max(4,(month.count/maxMonth)*140)}px`}} title={`${month.count} ${t("stats.watches")}`} /><span className="text-[10px] text-muted-foreground">{month.month.slice(5)}</span></div>)}</div></section>
      <section className="grid gap-4 lg:grid-cols-2"><article className="rounded-3xl border border-border bg-card/60 p-5"><h2 className="font-semibold">{t("stats.favouriteGenres")}</h2><div className="mt-4 grid gap-2">{stats.data.favouriteGenres.map((genre) => <div key={genre.name} className="flex justify-between rounded-xl border border-border px-3 py-2 text-sm"><span>{genre.name}</span><strong>{genre.count}</strong></div>)}</div></article><article className="rounded-3xl border border-primary/30 bg-primary/5 p-5"><p className="text-xs font-semibold uppercase tracking-widest text-primary">{t("stats.wrapped", { year: wrapped.data.year })}</p><p className="mt-3 font-display text-4xl font-bold">{hours(wrapped.data.minutes)}</p><p className="text-sm text-muted-foreground">{t("stats.activeDays", { count: wrapped.data.activeDays })}</p><div className="mt-4 grid gap-2 text-sm"><p>{wrapped.data.movies} {t("stats.films")} · {wrapped.data.episodes} {t("stats.episodes")}</p><p>{t("stats.favouriteGenre")} <strong>{wrapped.data.favouriteGenre ?? "—"}</strong></p>{wrapped.data.topTitles.map((item,index) => <p key={item.title}>{index+1}. {item.title} · {item.count}</p>)}</div></article></section>
    </div>
  );
}
