...
export const desktopService = {
  async initialize(): Promise<() => void> {
    ...
    try { cleanups.push(await onOpenUrl((urls) => { const route = urls.map(routeFromUrl).find(Boolean); if (route) navigate(route); })); } catch (error) { console.warn("Deep links unavailable", error); }
