...
export const progressRepository = {
  ...
  async listSeenMovies() {
    ...
    return db.select<Array<Record<string, unknown>>>("SELECT * FROM seen_movies ORDER BY watched_at DESC");
  },
  ...
  async getEpisodeProgress(seriesId: number): Promise<EpisodeProgress[]> {
    ...
    if (!db) {
      return browserStore.read().episodeProgress.filter((item) => item.seriesId === seriesId && item.watched);
    }
  ...
  async listTrackedSeries(): Promise<TrackedSeriesItem[]> {
    ...
    if (!db) {
      return browserStore.read().trackedSeries.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
    }
