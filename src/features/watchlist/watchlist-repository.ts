...
export const watchlistRepository = {
  async list(): Promise<WatchlistItem[]> {
    ...
    if (!db)
      return browserStore
        .read()
        .watchlist.filter((item) => storedProfile(item) === profile)
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    const rows = await db.select<Array<Record<string, unknown>>>(
      "SELECT * FROM profile_watchlist WHERE profile_id = $1 ORDER BY created_at DESC",
      [profile]
    );
  ...
  async has(mediaId: number, mediaType: WatchlistItem["mediaType"]): Promise<boolean> {
    ...
    if (!db)
      return browserStore
        .read()
        .watchlist.some(
          (item) => storedProfile(item) === profile && item.mediaId === mediaId && item.mediaType === mediaType
        );
    const rows = await db.select<Array<{ count: number }>>(
      "SELECT COUNT(*) count FROM profile_watchlist WHERE profile_id=$1 AND media_id=$2 AND media_type=$3",
  ...
  async upsert(item: WatchlistItem): Promise<void> {
    ...
    try {
      await db.execute(
        `INSERT OR REPLACE INTO profile_watchlist (profile_id,media_id,media_type,title,poster_path,backdrop_path,year,rating,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
        [
          profile,
          item.mediaId,
          item.mediaType,
  ...
  async remove(mediaId: number, mediaType: WatchlistItem["mediaType"]): Promise<void> {
    ...
    if (!db) {
      ...
      store.watchlist = store.watchlist.filter(
        (current) =>
          !(storedProfile(current) === profile && current.mediaId === mediaId && current.mediaType === mediaType)
      );
      browserStore.write(store);
      if (item)
        await historyRepository.add({
    ...
    await db.execute("BEGIN IMMEDIATE");
    try {
      await db.execute("DELETE FROM profile_watchlist WHERE profile_id=$1 AND media_id=$2 AND media_type=$3", [
        profile,
        mediaId,
        mediaType,
      ]);
