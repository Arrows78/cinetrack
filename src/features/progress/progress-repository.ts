...
export const progressRepository = {
  ...
  async listSeenMovies() {
    ...
    return db
      ? db.select<Array<Record<string, unknown>>>(
          "SELECT * FROM profile_seen_movies WHERE profile_id=$1 ORDER BY watched_at DESC",
          [profile]
        )
  ...
  async getEpisodeProgress(seriesId: number): Promise<EpisodeProgress[]> {
    ...
    if (!db)
      return browserStore
        .read()
        .episodeProgress.filter(
          (item) => storedProfile(item) === profile && item.seriesId === seriesId && item.watched
        );
