...
function review(row: Record<string, unknown>): CommunityReview {
  return {
    ...
    mediaType: row.media_type as CommunityReview["mediaType"], mediaId: Number(row.media_id),
    rating: row.rating == null ? null : Number(row.rating), body: String(row.body),
    containsSpoilers: Boolean(row.contains_spoilers), visibility: row.visibility as CommunityReview["visibility"],
...
export const communityRepository = {
  ...
  async getProfileByHandle(handle: string): Promise<CommunityProfile | null> {
    ...
    const { data, error } = await client.from("community_profiles").select("*").eq("handle", handle.toLowerCase()).maybeSingle();
    if (error) throw error;
    return data ? profile(data as Record<string, unknown>) : null;
  },
  ...
  async saveMyProfile(input: CommunityProfileInput): Promise<CommunityProfile> {
    const { client, user } = await authenticated();
    const { data, error } = await client.from("community_profiles").upsert({
      user_id: user.id,
  ...
  async acceptFollow(followerId: string): Promise<void> {
    const { client, user } = await authenticated();
    const { error } = await client.from("community_follows").update({ status: "accepted" })
  ...
  async unfollow(userId: string): Promise<void> {
    const { client, user } = await authenticated();
    const { error } = await client.from("community_follows").delete()
  ...
  async unblock(userId: string): Promise<void> {
    const { client, user } = await authenticated();
    const { error } = await client.from("community_blocks").delete()
  ...
  async unmute(userId: string): Promise<void> {
    const { client, user } = await authenticated();
    const { error } = await client.from("community_mutes").delete()
      .eq("muter_id", user.id).eq("muted_id", userId);
    if (error) throw error;
  },
  ...
  async publishReview(input: PublishReviewInput): Promise<CommunityReview> {
    ...
    const { data, error } = await client.from("community_reviews").upsert({
      user_id: user.id, media_type: input.mediaType, media_id: input.mediaId,
      rating: input.rating ?? null, body: input.body.trim(),
      contains_spoilers: input.containsSpoilers ?? false,
      visibility: input.visibility ?? "public", updated_at: now, deleted_at: null,
    }, { onConflict: "user_id,media_type,media_id" }).select("*").single();
    if (error) throw error;

    const mapped = review(data as Record<string, unknown>);
    await client.from("community_activities").insert({
      user_id: user.id, activity_type: "review_published", media_type: input.mediaType,
      media_id: input.mediaId, object_id: mapped.id, visibility: input.visibility ?? "public",
      payload: {},
    });
    return mapped;
  },
  ...
  async listReviews(mediaType: "movie" | "series", mediaId: number, limit = 30): Promise<CommunityReview[]> {
    const { client } = await authenticated();
    const { data, error } = await client.from("community_reviews").select("*")
      .eq("media_type", mediaType).eq("media_id", mediaId).is("deleted_at", null)
  ...
  async unlikeReview(reviewId: string): Promise<void> {
    const { client, user } = await authenticated();
    const { error } = await client.from("community_review_likes").delete()
  ...
  async addComment(reviewId: string, body: string): Promise<CommunityComment> {
    ...
    const { data, error } = await client.from("community_comments").insert({
      review_id: reviewId, user_id: user.id, body: body.trim(),
    }).select("*").single();
    if (error) throw error;
    return {
      id: data.id, reviewId: data.review_id, userId: data.user_id, body: data.body,
  ...
  async listComments(reviewId: string): Promise<CommunityComment[]> {
    const { client } = await authenticated();
    const { data, error } = await client.from("community_comments").select("*")
      .eq("review_id", reviewId).is("deleted_at", null).order("created_at", { ascending: true });
    if (error) throw error;
    return (data ?? []).map((row) => ({
      id: row.id, reviewId: row.review_id, userId: row.user_id, body: row.body,
  ...
  async feed(before?: string, limit = 30): Promise<CommunityActivity[]> {
    ...
    return ((data ?? []) as Array<Record<string, unknown>>).map((row) => ({
      ...
      payload: (row.payload ?? {}) as Record<string, unknown>, createdAt: String(row.created_at),
    }));
  },

  async notifications(limit = 50): Promise<CommunityNotification[]> {
    const { client, user } = await authenticated();
    const { data, error } = await client.from("community_notifications").select("*")
      .eq("user_id", user.id).order("created_at", { ascending: false }).limit(Math.min(Math.max(limit, 1), 100));
    if (error) throw error;
    return (data ?? []).map((row) => ({
      id: row.id, userId: row.user_id, actorId: row.actor_id,
      notificationType: row.notification_type, objectId: row.object_id,
  ...
  async markNotificationRead(notificationId: string): Promise<void> {
    const { client, user } = await authenticated();
    const { error } = await client.from("community_notifications")
      .update({ read_at: new Date().toISOString() }).eq("id", notificationId).eq("user_id", user.id);
    if (error) throw error;
  },
