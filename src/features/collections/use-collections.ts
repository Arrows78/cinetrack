...
export function useProfiles() {
  ...
  const create = useMutation({
    mutationFn: (name: string) => profileRepository.create(name),
    onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.local.profiles }),
  });
...
export function useCustomLists() {
  ...
  const create = useMutation({
    mutationFn: ({ name, description }: { name: string; description?: string }) =>
      customListRepository.create(name, description),
    onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.local.customLists }),
  });
  const remove = useMutation({
    mutationFn: (id: string) => customListRepository.remove(id),
    onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.local.customLists }),
  });
  return {
    ...query,
...
export function useCustomListItems(listId: string) {
  ...
  const remove = useMutation({
    mutationFn: ({ mediaId, mediaType }: { mediaId: number; mediaType: "movie" | "series" }) =>
      customListRepository.removeItem(listId, mediaId, mediaType),
    onSuccess: () => client.invalidateQueries({ queryKey: queryKeys.local.customList(listId) }),
  });
